...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      if (action === 'auth')     return await handleAuth(res);
      if (action === 'callback') return await handleCallback(req, res);
      if (action === 'spend')    return await handleSpend(req, res);
      return await handleListCampaigns(res);
    }

    if (req.method === 'PATCH') return await handleUpdateCampaign(req, res);

    return res.status(405).end();
  } catch (err: any) {
    console.error('[gads-campaigns] unhandled error', err);
